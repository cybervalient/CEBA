  ...
  for (iter in 2:ceda@stopCondition@iterations) {
    ...
    if(!globaling){
      ...
      if(syncrone){
        ...
        totalpopMatrix<- as.data.frame(matrix(nrow = totalpopLength, ncol = indSize))
        ...
        for (nRow in 1: ceda@latticeConfiguration@totalRows){
          for (nCol in 1: ceda@latticeConfiguration@totalCols){
            ...
            #Run EDA
            population <- new("Population",populationmatrix= popMatrix, fitness=array(fit))
            ...
            learning <- learning(ceda@learningOperator, convert.numeric(selPop@populationmatrix));
            
            sampMatrix <- sampling(ceda@samplingOperator,learning, convert.numeric(selPop@populationmatrix));
        ...
        #Print the statistic for the grid
        newPopulation <- new("Population",populationmatrix=totalpopMatrix,fitness=array(totalfitness));
