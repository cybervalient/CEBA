...
fitness.Weierstrass<-function(object){
  n <- length(object@x);
  a=0.5;b=3;
  kmax=20;
  fit=0;sum=0;s=0;
  for( i in 1:n){
    for(k in 0:kmax){
      sum=sum+((a^k)*cos(2*pi*(b^k)*(object@x[i]+0.5)));
    } 
    fit=fit+sum;
  }
  
  for(k in 0:kmax){
    s=s+(a^k)*cos(2*pi*(b^k)*(0.5))
  }
...
fitness.IsoPeaks <- function(object){
  ...
  if (object@x[1]==1 && object@x[2] == 1){
    Iso2 <- m;
  }
  i=3	
  while(i<=n){
    temp <- i+1
    if(object@x[i]==1 && object@x[temp]== 1){
      
      Iso1<-Iso1 +(m-1)
      
    }else if(object@x[i]==0 && object@x[temp]== 0){
